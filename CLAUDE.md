...
- Never write an E2E assertion that cannot fail. TC-WORK-04 stayed red for five CI rounds because the three assertions around it passed while the app was completely broken: `cy.location('pathname').should('include', '/worker')` is satisfied by `/worker/login`, `cy.get('body').should('be.visible')` is satisfied by an EMPTY body, and "no 24-hour times in the page text" is satisfied by a page with no text at all. A signed-out suite looked 2/3 green, so every round chased the one test that needed a real button instead of the login that had already failed. When one test is stubbornly red in an otherwise-green suite, suspect its neighbours first — assert the state you actually require (a specific path, a real control, `not.include` the login route), and give `cy.session` a `validate` so a cached session that no longer logs in is re-created rather than trusted
- Make the failure name its own cause before fixing anything. `cy.contains()` reports only what it could NOT find, which is why four rounds of inference went into a missing button; asserting by hand and printing what WAS on screen (every button label plus the page text) identified the real error — "Couldn't load your account / Auth session missing!" — on the first run that had it. Upload `cypress/screenshots` on failure too: an authenticated spec tests a DEPLOYED environment, so the failure cannot be reproduced from the checkout afterwards
